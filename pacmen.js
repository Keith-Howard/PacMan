...
function makePac() {
  ...
  newimg.width = 100;
  console.log('original position ' + position['x']);
...
function checkCollisions(item) {
  ...
  if ((item.position.x + item.velocity.x) >= windowWidth) {
  ...
  if (
    item.position.y + item.velocity.y > windowHeight ||
...
function makeOne() {
  console.log('Before the push callback');
